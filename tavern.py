...
def dialog_answer_buttons_1():
    keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Воу-воу! Палехчи, приятель!")]])
    return keyboard
...
def dialog_women():
    return 'Вы подходите к прекрасной незнакомке, в ее глазах играют задорные огоньки, кажется она не против уединиться с вами'

def dialog_women_buttons():
    keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Может поднимимся наверх?")],[KeyboardButton(text="Ну ее. Я не ищу продажной любви")]])
    return keyboard

def dialog_women_2():
    return 'Прекрасная незнакомка берет вас за руку и вдвоем вы идете в маленькую комнатку для утех. Там, она страсно стягивает с вас штаны, снимает с себя платье... Панталоны... О, нет! Its a TRAP!!!'

def description_tawern():
    return 'Вы вновь окидываете комнату взглядом. В дальней части от вас расположена стойка, которую протирает крупный рыжебородый дварф в переднике и с засученными рукавами, завидев вас, он делает приветственный жест рукой, приглашая присесть за стойкой. В ближнем темном углу спиной к вам сидит широкоплечий мужчина, темный волосы и зеленая жилетка на нем подсказывает вам, что это может быть слуга лорда Логен. Надо сказать, что в столь ранний час он тут не единственный посетитель.'

def dialog_women_buttons2():
    keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Бежать!!!")]])
    return keyboard

def description_tawern_run():
    return 'Черт возьми! Что это было? У нее.. Там.. Вы с трудом ускользнули от "Прекрасной дамы" на площадь'